...
% --- AI MONOPOLIST TAKE RATE ON THE WHOLE TASK BLOCK ---
lambda = 0.1;
...
% Find A0_A such that, at its private-return steady state, Country A hits
% the target output ratio relative to Country B.
...
% D. Base savings rates consistent with the code's GNI concept
% In autarky with leader-owned AI:
%   GNI_B = Y_net_B - delta*K_B = (1 - lambda*(1-gamma))*Y_B - delta*K_B
%   GNI_A = Y_net_A - delta*K_A + AI_world
%         = (1 - lambda*(1-gamma))*Y_A - delta*K_A + lambda*(1-gamma)*(Y_A + Y_B)
%         = Y_A - delta*K_A + lambda*(1-gamma)*Y_B
y_A_ss = get_y(k_ss_A, b_start, rho, gamma, A0_A, 1);
ai_rent_A_ss = get_ai_rent(k_ss_A, b_start, rho, gamma, A0_A, 1, lambda);
ai_rent_B_ss = get_ai_rent(k_ss_B, b_start, rho, gamma, A0_B, 1, lambda);

GNI_B_ss = (y_B_ss - ai_rent_B_ss) - delta * k_ss_B;
GNI_A_ss = (y_A_ss - ai_rent_A_ss) - delta * k_ss_A + (ai_rent_A_ss + ai_rent_B_ss);

s_base_A = (delta * k_ss_A) / GNI_A_ss;
s_base_B = (delta * k_ss_B) / GNI_B_ss;
...
for j = 1:4
    ...
    ai_rev_pct_gni_v = zeros(2, length(t));
    ...
    for i = 1:length(t)
        ...
        % Add AI monopoly income to the Leader (Country A) as external property income.
        GNI_v(1,i) = GNI_v(1,i) + sum(ai_rent_v(:,i));
        ai_rev_pct_gni_v(:,i) = 100 * ai_rent_v(:,i) ./ max(abs(GNI_v(:,i)), 1e-12);
        ...
        GNI_parts_all(j, 1, i, 2) = P(1,1) * r_priv_v(1,i) / L_A;
        GNI_parts_all(j, 1, i, 3) = P(1,2) * (r_priv_v(2,i) - w_vec(2) - tau_vec(1)) / L_A;


        % Country B
        GNI_parts_all(j, 2, i, 1) = labor_inc(2) / L_B;
        GNI_parts_all(j, 2, i, 2) = P(2,2) * r_priv_v(2,i) / L_B;
        GNI_parts_all(j, 2, i, 3) = P(2,1) * (r_priv_v(1,i) - w_vec(1) - tau_vec(2)) / L_B;
        ...
        if i < length(t)
            ...
            if i == 1
                ...
                g_v(:,i) = (Y_v(:,i) - Y_v(:,i-1)) ./ max(abs(Y_v(:,i-1)), 1e-12);
            end
...
AI_income_all(j, :, :) = ai_rent_v(:, 1:T_sim);
rgA = rg_v(1, t_p) * 100;
rgB = rg_v(2, t_p) * 100;
...
row_ylims = { ...
    ...
    [min([offA, offB]), max([offA, offB]) * 1.1]};
...
for s = 1:4
    leader_ai_income = squeeze(sum(AI_income_all(s, :, 1:T_sim), 2));   % T_sim x 1
    ...
    for c = 1:2
        ...
        base_levels = squeeze(GNI_parts_all(s, c, 1:T_sim, :));   % T_sim x 3
        leader_ai_income = squeeze(sum(AI_income_all(s, :, 1:T_sim), 2));
        leader_ai_income = leader_ai_income(:);

        if c == 1
            ai_layer = leader_ai_income / L_A;
        else
            ai_layer = zeros(T_sim,1);
        end
       
        
        data_levels = [base_levels, ai_layer];
...
lgd = legend(h, {'Labor Income', 'Domestic Cap Inc', 'Foreign Cap Inc', 'AI Income'}, ...
    'Orientation', 'horizontal');
set(lgd, 'Position', [0.28, 0.02, 0.45, 0.03]);
%% --- HELPERS ---
...
function ai_rent = get_ai_rent(k, bt, rho, gamma, A, L, lambda)
    % AI monopolist revenue: a common take rate on the whole task block.
    y = get_y(k, bt, rho, gamma, A, L);
    ai_rent = lambda .* (1-gamma) .* y;
end


function r = get_r_private(k, bt, rho, gamma, A, L, d_val, lambda)
    % Private net return consistent with net output
    % Y_net = [1 - lambda*(1-gamma)] * Y.
    scale = 1 - lambda .* (1-gamma);
    mpk_gross = get_r(k, bt, rho, gamma, A, L, 0);
    r = scale .* mpk_gross - d_val;
end
