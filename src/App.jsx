...
import {
  ...
  ChevronRight, Database, TrendingDown, BookOpen, Zap, Timer, PauseCircle, LayoutGrid
} from 'lucide-react';
...
const App = () => {
  ...
  const allScenariosResults = useMemo(() => {
    ...
    const target_ratios = n === 2 ? [1.5] : [2.0, 1.5];
    ...
    for (let i = 0; i < n - 1; i++) {
      A0_vec[i] = fzero((a) => get_y(solve_ki_for_r(r_target, b_start, delta, a, 1.0, gamma, rho), b_start, rho, gamma, a, 1.0) - (y_ss_num * target_ratios[i]), 0.1, 10.0);
    }

    const K_init_pc = A0_vec.map(a => solve_ki_for_r(r_target, b_start, delta, a, 1.0, gamma, rho));
    const Y_init_pc_vals = K_init_pc.map((ki, i) => get_y(ki, b_start, rho, gamma, A0_vec[i], 1.0));
    const s_base_vec = K_init_pc.map((ki, i) => (delta * ki) / Y_init_pc_vals[i]);
    ...
    return SCENARIOS.map(scen => {
      ...
      if (scen.id === 'hype') {
        ...
        beta_paths[0] = bA4; beta_paths[1] = bA4.map(v => 0.1 * v);
      ...
      for (let t = 0; t < T_sim; t++) {
        ...
        const GNI_parts = Array.from({length: n}, (_, k) => {
          const dom_cap = P[k][k] * r_real[k];
          let for_cap = 0;
          for (let m = 0; m < n; m++) if (m !== k) for_cap += P[k][m] * (r_real[m] - w_vec[m]);
          return { labor: labor_inc[k] / L_vec[k], dom_cap: dom_cap / L_vec[k], for_cap: for_cap / L_vec[k] };
        });
        ...
        for (let iter = 0; iter < 40; iter++) {
          ...
          const r_yield = V_proj.map((_, owner) => {
            let yieldVal = 0;
            for (let loc = 0; loc < n; loc++) yieldVal += (K_f[loc] / V_world) * (rr_f[loc] - (owner !== loc ? w_vec[loc] : 0));
            return yieldVal;
          });
        ...
        history.push({
          t, beta1: bt_r[0], beta2: bt_r[1], beta3: bt_r[2] || 0, betaP: scen.id === 'hype' ? bP_L_ext[t] : null,
          Y1: (Y[0] / (Y_init_pc_vals[0] * L_vec[0])) * 100, Y2: (Y[1] / (Y_init_pc_vals[1] * L_vec[1])) * 100, Y3: n === 3 ? (Y[2] / (Y_init_pc_vals[2] * L_vec[2])) * 100 : 0,
          r1: r_real[0] * 100, r2: r_real[1] * 100, r3: (r_real[2] || 0) * 100, s1: s_guess[0], s2: s_guess[1], s3: s_guess[2] || 0,
          sh1: (v_pc[0] / v_pc.reduce((a,b)=>a+b,0)) * 100, sh2: (v_pc[1] / v_pc.reduce((a,b)=>a+b,0)) * 100, sh3: n === 3 ? (v_pc[2] / v_pc.reduce((a,b)=>a+b,0)) * 100 : 0,
      ...
      return { id: scen.id, name: scen.name, history };
    });
  }, [mode, params]);

  const activeScenarioResults = useMemo(() => allScenariosResults.find(r => r.id === activeScenario), [activeScenario, allScenariosResults]);
    ...
    <div className="flex h-screen bg-slate-100 overflow-hidden font-sans text-slate-900">
      <aside className="w-80 bg-white border-r border-slate-300 flex flex-col shadow-xl z-20">
        <div className="p-4 border-b border-slate-200 flex items-center space-x-3 bg-slate-900 text-white">
          <Globe className="w-5 h-5 text-blue-400" /><h1 className="font-black text-[11px] tracking-[0.2em] uppercase">Simulation App</h1>
        </div>
        <div className="flex-1 overflow-y-auto p-5 space-y-8 scrollbar-hide">
          <div className="flex bg-slate-100 p-1 rounded-lg">
            {['dashboard', 'decomposition', 'about'].map(v => (
          ...
          {activeView !== 'about' && (
            <>
              ...
              <div className="space-y-6 pt-2 border-t border-slate-100">
                ...
                {activeScenario === 'logjam' && <ParamSlider label="Logjam Lag" val={params.plateau} min={0} max={20} step={1} onChange={v => setParams({...params, plateau: v})} icon={<PauseCircle className="w-3 h-3 text-rose-500" />} />}
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-10 bg-white border-b border-slate-300 px-6 flex items-center justify-between shadow-sm">
          <h2 className="text-[10px] font-black uppercase tracking-[0.2em]">{activeView.toUpperCase()} VIEW</h2>
            ...
            <>
              {showExplainers && (
                <div className="mb-6 grid grid-cols-3 xl:grid-cols-6 gap-3">
                  <ExplainerCard title="Gestation" text="Delay between investment and productive capital." color="border-blue-500" />
                  <ExplainerCard title="Diffusion Lag" text="Time for innovations to spread from the Leader." color="border-indigo-500" />
                  <ExplainerCard title="Logjam Lag" text="Stall period between adoption waves." color="border-rose-500" />
                  <ExplainerCard title="Growth Rates" text="Productivity growth paths for each country." color="border-emerald-500" />
                  <ExplainerCard title="Sensitivity" text="How savings respond to return spreads." color="border-amber-500" />
                  <ExplainerCard title="Profit Tax" text="Tax on foreign earnings in a specific country." color="border-rose-500" />
                </div>
              )}
              <div className="grid grid-cols-2 gap-4 pb-10">
                <ChartBlock title="Tech Adoption (β)"><ResponsiveContainer width="100%" height={140}><LineChart data={activeScenarioResults.history} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="t" fontSize={8} /><YAxis fontSize={8} domain={['auto', 'auto']} /><Tooltip content={<SimpleTooltip />} /><Line type="monotone" dataKey="beta1" name="Leader" stroke="#0000ff" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="beta2" name="Follower" stroke="#ff0000" strokeWidth={1.5} strokeDasharray="5 5" dot={false} />{mode === '3C' && <Line type="monotone" dataKey="beta3" name="Laggard" stroke="#00aa00" strokeWidth={1.5} strokeDasharray="3 3" dot={false} />}{activeScenario === 'hype' && <Line type="monotone" dataKey="betaP" name="Perceived" stroke="#0000ff" strokeWidth={1} strokeDasharray="2 2" opacity={0.3} dot={false} />}</LineChart></ResponsiveContainer></ChartBlock>
                <ChartBlock title="Output Index (Y)"><ResponsiveContainer width="100%" height={140}><LineChart data={activeScenarioResults.history} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="t" fontSize={8} /><YAxis fontSize={8} domain={['auto', 'auto']} /><Line type="monotone" dataKey="Y1" stroke="#0000ff" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="Y2" stroke="#ff0000" strokeWidth={1.5} dot={false} />{mode === '3C' && <Line type="monotone" dataKey="Y3" stroke="#00aa00" strokeWidth={1.5} dot={false} />}</LineChart></ResponsiveContainer></ChartBlock>
                <ChartBlock title="Realized Returns (%)"><ResponsiveContainer width="100%" height={140}><LineChart data={activeScenarioResults.history} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="t" fontSize={8} /><YAxis fontSize={8} domain={['auto', 'auto']} /><Line type="monotone" dataKey="r1" stroke="#0000ff" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="r2" stroke="#ff0000" strokeWidth={1.5} strokeDasharray="5 5" dot={false} />{mode === '3C' && <Line type="monotone" dataKey="r3" stroke="#00aa00" strokeWidth={1.5} strokeDasharray="3 3" dot={false} />}</LineChart></ResponsiveContainer></ChartBlock>
                <ChartBlock title="Savings Rate (s)"><ResponsiveContainer width="100%" height={140}><LineChart data={activeScenarioResults.history} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="t" fontSize={8} /><YAxis fontSize={8} domain={['auto', 'auto']} /><Line type="monotone" dataKey="s1" stroke="#0000ff" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="s2" stroke="#ff0000" strokeWidth={1.5} strokeDasharray="5 5" dot={false} />{mode === '3C' && <Line type="monotone" dataKey="s3" stroke="#00aa00" strokeWidth={1.5} dot={false} />}</LineChart></ResponsiveContainer></ChartBlock>
                <ChartBlock title="Wealth Share % (p.c.)"><ResponsiveContainer width="100%" height={140}><LineChart data={activeScenarioResults.history} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="t" fontSize={8} /><YAxis fontSize={8} domain={['auto', 'auto']} /><Line type="monotone" dataKey="sh1" stroke="#0000ff" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="sh2" stroke="#ff0000" strokeWidth={1.5} strokeDasharray="5 5" dot={false} />{mode === '3C' && <Line type="monotone" dataKey="sh3" stroke="#00aa00" strokeWidth={1.5} strokeDasharray="3 3" dot={false} />}</LineChart></ResponsiveContainer></ChartBlock>
                <ChartBlock title="Labour Share GNI"><ResponsiveContainer width="100%" height={140}><LineChart data={activeScenarioResults.history} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="t" fontSize={8} /><YAxis fontSize={8} domain={['auto', 'auto']} /><Line type="monotone" dataKey="ls1" stroke="#0000ff" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="ls2" stroke="#ff0000" strokeWidth={1.5} dot={false} />{mode === '3C' && <Line type="monotone" dataKey="ls3" stroke="#00aa00" strokeWidth={1.5} dot={false} />}</LineChart></ResponsiveContainer></ChartBlock>
                <ChartBlock title="Break-even Gap p.c."><ResponsiveContainer width="100%" height={140}><LineChart data={activeScenarioResults.history} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="t" fontSize={8} /><YAxis fontSize={8} domain={['auto', 'auto']} /><ReferenceLine y={0} stroke="#475569" strokeWidth={1} /><Line type="monotone" dataKey="be1" stroke="#0000ff" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="be2" stroke="#ff0000" strokeWidth={1.5} dot={false} />{mode === '3C' && <Line type="monotone" dataKey="be3" stroke="#00aa00" strokeWidth={1.5} strokeDasharray="3 3" dot={false} />}</LineChart></ResponsiveContainer></ChartBlock>
                <ChartBlock title="GNI Index (Base=100)"><ResponsiveContainer width="100%" height={140}><LineChart data={activeScenarioResults.history} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="t" fontSize={8} /><YAxis fontSize={8} domain={['auto', 'auto']} /><Line type="monotone" dataKey="GNI1" stroke="#0000ff" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="GNI2" stroke="#ff0000" strokeWidth={1.5} dot={false} />{mode === '3C' && <Line type="monotone" dataKey="GNI3" stroke="#00aa00" strokeWidth={1.5} dot={false} />}</LineChart></ResponsiveContainer></ChartBlock>
              </div>

              {/* GNI DECOMPOSITION SECTION */}
              <div className="mt-12 border-t border-slate-200 pt-8 pb-10">
                <div className="mb-6">
                  <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight flex items-center">
                    <LayoutGrid className="w-5 h-5 mr-2 text-indigo-600" /> GNI Decomposition ({activeScenarioResults.name})
                  </h3>
                </div>
                <div className={`grid gap-4 ${mode === '2C' ? 'grid-cols-2' : 'grid-cols-3'}`}>
                  {[...Array(mode === '2C' ? 2 : 3)].map((_, c) => (
                    <div key={`dashboard-dec-${c}`} className="bg-white p-3 border border-slate-300 rounded-sm shadow-sm">
                      ...
                      <ResponsiveContainer width="100%" height={150}>
                        <AreaChart data={activeScenarioResults.history.map(h => ({ t: h.t, ...h.gni_parts[c] }))} margin={{ top: 5, right: 5, left: -25, bottom: 5 }}>
                          <CartesianGrid strokeDasharray="3 3" vertical={false} />
                          <XAxis dataKey="t" fontSize={7} />
                          <YAxis fontSize={7} domain={[0, 'auto']} />
                          <Area type="monotone" dataKey="labor" stackId="1" stroke="#cc4c4c" fill="#cc4c4c" opacity={0.8} />
                          <Area type="monotone" dataKey="dom_cap" stackId="1" stroke="#4c4ccc" fill="#4c4ccc" opacity={0.8} />
                          <Area type="monotone" dataKey="for_cap" stackId="1" stroke="#4ccc4c" fill="#4ccc4c" opacity={0.8} />
                        </AreaChart>
                      </ResponsiveContainer>
                    </div>
                  ))}
          ...
          {activeView === 'decomposition' && (
            <div className="space-y-12 pb-20">
              <div className="mb-8 border-b border-slate-200 pb-4 flex justify-between items-end">
                <div>
                  <h2 className="text-xl font-black text-slate-800 uppercase tracking-widest">{activeScenarioResults.name} Decomposition</h2>
                  <p className="text-sm text-slate-500 mt-1 italic">Individual breakdown of GNI per capita sources.</p>
                </div>
              </div>
              <div className={`grid gap-6 ${mode === '2C' ? 'grid-cols-2' : 'grid-cols-3'}`}>
                {[...Array(mode === '2C' ? 2 : 3)].map((_, c) => (
                  <div key={`full-dec-${c}`} className="bg-white p-6 border border-slate-300 rounded-lg shadow-md">
                    <div className="flex justify-between items-center border-b border-slate-100 pb-3 mb-4">
                      <h4 className="text-sm font-black uppercase text-indigo-600">Country {String.fromCharCode(65 + c)}</h4>
                    </div>
                    <ResponsiveContainer width="100%" height={250}>
                      <AreaChart data={activeScenarioResults.history.map(h => ({ t: h.t, ...h.gni_parts[c] }))} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                        <CartesianGrid strokeDasharray="3 3" vertical={false} />
...
const GNILegend = () => (
  <div className="flex justify-center items-center space-x-10 py-6 mt-8 border-t border-slate-200">
    <div className="flex items-center space-x-3">
      <div className="w-4 h-4 bg-[#cc4c4c] rounded-sm shadow-sm"></div>
      <span className="text-[11px] font-black text-slate-700 uppercase tracking-widest">Labor Income</span>
    </div>
    <div className="flex items-center space-x-3">
      <div className="w-4 h-4 bg-[#4c4ccc] rounded-sm shadow-sm"></div>
      <span className="text-[11px] font-black text-slate-700 uppercase tracking-widest">Domestic Cap Inc</span>
    </div>
    <div className="flex items-center space-x-3">
      <div className="w-4 h-4 bg-[#4ccc4c] rounded-sm shadow-sm"></div>
      <span className="text-[11px] font-black text-slate-700 uppercase tracking-widest">Foreign Cap Inc</span>
    </div>
  </div>
);

const ExplainerCard = ({ title, text, color }) => (
  <div className={`bg-slate-900 text-white p-3 rounded shadow-lg border-l-4 ${color}`}><div className="flex items-center space-x-2 mb-1 text-blue-400"><span className="text-[8px] font-black uppercase tracking-widest">{title}</span></div><p className="text-[9px] leading-tight text-slate-300">{text}</p></div>
);
...
const AboutPage = () => (
  <div className="max-w-4xl mx-auto bg-white p-10 shadow-2xl space-y-12 rounded-sm mb-10">
    ...
    <section><h3 className="text-lg font-black text-blue-600 mb-4 uppercase flex items-center tracking-tight"><ChevronRight className="w-5 h-5 mr-1" /> Scenario Analysis</h3><div className="grid grid-cols-2 gap-4">
      <AboutCard title="Hype Cycle" text="A 'bubble' where tech expectations outpace reality early on, causing spikes in returns before a correction." />
      <AboutCard title="Tidal Flow" text="The baseline. Tech adopts in an S-curve, with diffusion lags between countries pulling capital back and forth." />
      <AboutCard title="Logjam" text="A structural stall where progress hits a plateau after initial adoption, creating stagnation before a second wave." />
      <AboutCard title="The Gulf" text="Permanent divergence where followers fail to adopt, leading to massive, long-term capital flight to the leader." />
    </div></section>
...
const AboutCard = ({ title, text }) => (<div className="p-5 bg-slate-50 border border-slate-200"><h4 className="font-black text-xs uppercase text-slate-800 mb-2">{title}</h4><p className="text-[11px] text-slate-500 leading-relaxed">{text}</p></div>);
const Mechanism = ({ title, text }) => (<div className="flex items-start space-x-4"><div className="w-1.5 h-1.5 rounded-full bg-blue-400 mt-2 flex-shrink-0" /><div><p className="font-black text-slate-800 text-xs uppercase">{title}</p><p className="text-[11px] text-slate-500">{text}</p></div></div>);
...
const ParamSlider = ({ label, val, min, max, step, onChange, icon }) => (
  <div className="space-y-1.5">
    <div className="flex justify-between items-center text-[8px] font-black text-slate-600 tracking-tight"><div className="flex items-center space-x-1">{icon}<span>{label.toUpperCase()}</span></div><span className="font-mono text-blue-700 bg-blue-50 px-1 rounded">{val}</span></div>
...
const SimpleTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    const formattedLabel = payload[0]?.name ? "" : `T = ${label}`;
    return (
      <div className="bg-white border border-slate-300 p-2 text-[8px] font-bold shadow-xl">
        {formattedLabel && <div className="mb-1 text-slate-400 text-[10px]">{formattedLabel}</div>}
        {payload.map((entry, i) => (
