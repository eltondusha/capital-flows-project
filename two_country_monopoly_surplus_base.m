...
% Under the surplus-based AI rent, investors keep the beta=0 benchmark MPK
% plus only a (1-lambda) share of the AI-enabled increment.
...
% Evaluate steady-state GNI using the same accounting as the main loop:
% labor income + private capital income, with total AI rent assigned to the Leader.
...
r_priv_A_ss = get_r_private(k_ss_A, b_start, rho, gamma, A0_A, 1, delta, lambda);
r_priv_B_ss = get_r_private(k_ss_B, b_start, rho, gamma, A0_B, 1, delta, lambda);

labor_A_ss = (y_A_ss - ai_rent_A_ss) - (r_priv_A_ss + delta) * k_ss_A;
labor_B_ss = (y_B_ss - ai_rent_B_ss) - (r_priv_B_ss + delta) * k_ss_B;
cap_A_ss = k_ss_A * r_priv_A_ss;
cap_B_ss = k_ss_B * r_priv_B_ss;

GNI_B_ss = labor_B_ss + cap_B_ss;
GNI_A_ss = labor_A_ss + cap_A_ss + (ai_rent_A_ss + ai_rent_B_ss);
